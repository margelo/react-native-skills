...
- Use `setOn...Callback(callback | undefined)` only when replacement is the intended semantic: the object has exactly one callback slot, the callback is more like configuration than event subscription, or a hot-path native pipeline cannot reasonably multiplex listeners. Document that it replaces the previous callback and that `undefined` removes it.
- Avoid naming a replacement API `setOn...Listener` unless the surrounding API already uses that convention. "Listener" usually implies additive subscription ownership; "Callback" is clearer for a single replaceable slot.
