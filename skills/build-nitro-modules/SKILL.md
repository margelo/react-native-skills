...
- Remember that TypeScript optional fields become native optionals (`T?` / `std::optional<T>`) in generated Swift, Kotlin, and C++. Do not use optional Nitro fields merely to express public defaults when a TypeScript facade can normalize those defaults before calling native code.
- Prefer resolved native option structs for Nitro methods that cross into Swift/Kotlin/C++. Public JS wrappers may accept ergonomic optional options, but generated Nitro structs should avoid optional fields unless absence is a real native state.
...
- Put Kotlin and Swift extension functions/properties in named files with internal/package visibility unless they are tiny and only serve the file's primary type. A `Hybrid*Factory.kt` or `Hybrid*.swift` file should not end with pages of barcode-format mappings, companion extensions, or platform conversion utilities.
- Split unrelated native conversions into one focused extension file per source/target conversion, such as `Barcode+toScannedCode.kt` and `TargetBarcodeFormat+toMLKitFormat.kt`. Do not hide a trivial `map { it.toX() }` behind a concrete collection extension like `Array<TargetBarcodeFormat>.toMLKitFormats()`.
