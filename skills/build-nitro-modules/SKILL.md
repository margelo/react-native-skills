...
- Use `setOn...Callback(callback | undefined)` only for single hot-path callbacks owned by an object, where replacing or removing the callback is the natural operation. The name should say `Callback`, not `Listener`, because it is one replaceable slot rather than an additive subscription.
