...
- Split large specs and type surfaces into focused files under `src/specs/` or nearby type folders. Prefer one `.nitro.ts` spec per major HybridObject or domain instead of a single file containing every option, result, event, and helper type.
