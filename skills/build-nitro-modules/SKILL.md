...
- Put native conversion helpers on the smallest meaningful type. Do not add array/list/vector conversion helpers that only wrap a standard `map`/`transform`; keep that one-liner at the call site unless the collection conversion adds real behavior.
